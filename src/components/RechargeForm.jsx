...
const RechargeForm = ({ operators = [] }) => {
    ...
    const handleInputChange = (e) => {
        ...
        // Show plans when operator is selected
    ...
    const validateForm = () => {
        ...
        if (!formData.circle) {
            newErrors.circle = 'Please select a circle';
        }

        if (!formData.amount) {
            newErrors.amount = 'Please enter or select an amount';
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {operatorList.map((op) => (
                                <button
                                    key={op.id}
                                    type="button"
                                    onClick={() => handleInputChange({ target: { name: 'operator', value: op.id } })}
                                    className={`p-4 rounded-lg border-2 transition-all duration-200 ${formData.operator === op.id
                    ...
                    <div>
                        ...
                        {errors.circle && (
                            <p className="text-red-500 text-sm mt-1">{errors.circle}</p>
                        )}
                    </div>
                        ...
                        <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                            {quickAmounts.map((amount) => (
                                <button
                                    ...
                                    className={`py-3 rounded-lg font-semibold transition-all duration-200 ${formData.amount === amount.toString()
                    ...
                    {/* Available Plans */}
                    {showPlans && plans[formData.operator] && (
                        <div className="animate-fadeIn">
                            <label className="block text-sm font-semibold mb-3 text-gray-700 dark:text-gray-300">
                                Popular Plans
                            </label>
                            <div className="grid md:grid-cols-2 gap-4">
                                {plans[formData.operator].map((plan) => (
                                    <div
                                        key={plan.id}
                                        onClick={() => handlePlanSelect(plan)}
                                        className={`p-4 rounded-lg border-2 cursor-pointer transition-all duration-200 ${formData.plan?.id === plan.id
